<template>
  <div style="height: 78vh;width: 100vw;">
    ...
    <section class="content">
      <mu-row>
        <!-- 左边 -->
        <mu-col  span="12" sm="12" md="8" lg="8" xl="8" style="display: flex;height: 78vh;flex-direction: column;">
          ...
          <div class="left-item" style="display:flex;flex-grow:2"  v-for="(item,index) in exBigTypeList"  :key="index" >
            ...
            <div class="left-item-title" v-for="(item2,idx) in status" :key="idx">
              <div  class="left-item-circle" ref="circleSize"
                :class="circleStyle(item.exCurrentFormCode,idx)"  @click="doOperate(item,idx,index)"
              ></div>
            </div>
          </div>
        </mu-col>
        ...
        <!-- 右边 -->
        <mu-col span="12" sm="12" md="4" lg="4" xl="4" style="display: flex;height: 78vh;flex-direction: column;">
          <div class="right-item right-item-firth" style="display:flex;flex-grow:1">
            <!-- style="font-size: 3.2vw;" -->
            <div style="font-size: 3.2vw;">
              <div>运行状态</div>
                <!-- liuQian去掉 只保留中文 2020-0402-->
              <!-- <div>Product State</div> -->
            </div>
          </div>
          <div class="right-item right-item-second" :class="rightRunstatusClass" style="display:flex;flex-grow:3">
            <div>
          ...
          <div class="right-item right-item-third" style="display:flex;flex-grow:6">
            <div style="display: flex;flex-direction: column;height: 55vh;">
              <mu-row class="right-item-line">
                <mu-col span="5" fill>异常类型代码:</mu-col>
                <mu-col span="7" fill style="display: flex;" >{{currentData2.exTypeCode ? currentData2.exTypeCode : ""}}</mu-col>
              </mu-row>
              <mu-row class="right-item-line">
                <mu-col span="5" fill>异常类型名称:</mu-col>
                <mu-col span="7" fill style="display: flex;" >{{currentData2.exTypeName ? currentData2.exTypeName : ""}}</mu-col>
              </mu-row>
              <mu-row class="right-item-line">
                <!--2020-0325 异常内容编码: -->
                <mu-col span="5" fill>异常单据编码:</mu-col>
                <mu-col span="7" fill>{{currentData2.usmBillCode | ellipsis}}</mu-col>
              </mu-row>
              <mu-row class="right-item-line">
                <!--2020-0325 异常内容名称: {{currentData2.launchDate ? currentData2.launchDate : ""}}-->
                <mu-col span="5" fill>单据发起时间:</mu-col>
                <mu-col span="7" fill>{{currentData2.launchDate | ellipsis }}</mu-col>
              </mu-row>
            </div>
          </div>
        </mu-col>
    ...
    <!--响应弹框-->
    <mu-container>
      <mu-dialog title="响应"  width="500" max-width="500" :padding="padding" scrollable :open.sync="openScroll" >
        <mu-row>
          <mu-col span="12">
            <mu-select label="请选择异常小类" v-model="currentData.exTypeName" full-width @change="smallTypeChangeEvt">
              <mu-option v-for="(item,index) in exTypeList" :key="index"
          ...
          <mu-col span="12">
            <mu-select label="请选择异常异常原因" v-model="currentData.exContentName" full-width @change="contentChangeEvt">
              <mu-option
...
export default {
  name: "UsmAndon",
  props: ['workCell','workCenter','trackOrderId','equip','tool'],
  filters: {
    //定义过滤器
    ellipsis(value) {
      if (!value) return "";
      if(value.length == 26){
        return value.slice(0, 6) + "..." + value.slice(value.length-10, value.length);
  ...
  data() {
    return {
      workCell2:'',
      workCenter2:'',
      loginName: Storage.getObject("user").username,// Storage.getObject("loginConfig").un
      currentBigType:'',//当前点击的异常大类【设备 物料 生产 质量】
  ...
  methods: {
    ...
    doOperate(item, idx, index) {
      this.currentBigType=item.exForignerName;
      ...
      if (idx == 0) {
        ...
        //2020-03-24 junke暂定规则:不弹出选择异常小类,直接触发异常发起，建立异常单据
    ...
    //2020-03-24 原有无 item参数项
    dealFormData(item) {
      let returnData = {};
      let user = Storage.getObject("user");

      // 工作中心
      let workCenterId = this.workCenter.workCenterId;
      let workCenterCode = this.workCenter.workCenterCode;

      // 工位
      let workCellId = this.workCell.workCellId;
      let workCellCode = this.workCell.workCellCode;

      // 异常内容
      // let exContentId = this.currentData.exContent.usmContentId;
      // let exContentCode = this.currentData.exContent.usmContentCode;
      // let exContentName = this.currentData.exContent.usmContentName;
      let exContentId = item.contentId;
      let exContentCode = item.contentCode;
      let exContentName = item.contentName;
      ...
      let exTypeId = item.exTypeId;
      let exTypeCode = item.exTypeCode;
      let exTypeName = item.exTypeName;
...
//2020-03-25派工单任务主键 和 机台信息
      form.append("trackOrderId", this.trackOrderId);
      form.append("deviceCode", this.equip);
...
//2020-03-25派工单任务主键
      let trackOrderId=this.trackOrderId;
    ...
    //异常发起(响应操作)
    doLanch(item) {
      let form = this.dealFormData(item);
      if (form.flag) {
        Toast.warning({ message: form.msg, close: false });
        return false;
      }

      //创建设备维修任务单据
      if(item.exTypeName.indexOf("设备")!=-1){//this.currentBigType=='Machine'
        this.createEquipWxBill();
      }

      //创建模具维修任务单据
      if(item.exTypeName.indexOf("模具")!=-1){//this.currentBigType=='Machine'
        this.createMouldWxBill();
      }
      ...
      http.post({
        ...
        .then(res => {
          if (res.status == 200 && res.data.erroCode == 0) {
            ...
            //2020-03-25 暂定改为  异常单据编码和单据发起时间
            this.currentData2.exTypeCode = item.exTypeCode;
            this.currentData2.exTypeName = item.exTypeName;
            ...
            // this.currentData2.exContentCode = this.currentData.exContent? this.currentData.exContent.usmContentCode: "";
            // this.currentData2.exContentName = this.currentData.exContent? this.currentData.exContent.usmContentName: "";
          } else {
            Toast.warning({message: res.data.msg,position: "bottom",close: false});
          }
    ...
    clearRightData() {
      ...
      //2020-0325 清除
      this.currentData2.launchDate = "";
      this.currentData2.usmBillCode = "";
    },
    // 工作中心,工位查询
    selectWorkCell() {
      let loginName = this.loginName;
    ...
    createEquipWxBill() {
      let loginName = this.loginName;
      let data = qs.stringify({ loginName, workCellId: this.workCell.workCellId});
      http.post({
          url: "productPadCommController!createEquipWxBill.m",
          data: data,
          autoLogin: true
        })
        .then(res => {
    ...
    createMouldWxBill() {
      let loginName = this.loginName;
      let data = qs.stringify({ loginName, workCellId: this.workCell.workCellId,tool:this.tool});
      http.post({
          url: "productPadCommController!createMouldWxBill.m",
          data: data,
          autoLogin: true
        })
        .then(res => {
          ...
          if (res.status == 200 && res.data.erroCode == 0) {
            ...
            resData.forEach((item, i) => {
              if (item.launchDate == timeList[0]) {
                ...
                //2020-0325当前最新的异常单据号 和发起时间
                // console.log("this.currentData2.usmBillCode========="+item.usmBillCode);
                // console.log("this.currentData2.launchDate========="+item.launchDate);
                this.currentData2.usmBillCode = item.usmBillCode;
                this.currentData2.launchDate = item.launchDate;
              }
            });
            if(timeList.length==0){
              this.clearRightData();
            ...
            let resRunstatus = this.rightStutsStyle();
                  // console.log("红0|黄1|绿色2==="+resRunstatus);
            ...
            // console.log("self.workCell.id ==="+self.workCell.id);
            if (self.currentIndex === "" &&  self.workCell.id === undefined){
              // 初始化页面全部变成白色
            ...
            } else if (self.currentIndex === "" &&  self.workCell.id !== undefined){
            ...
            if (self.currentIndex !== "") {
              ...
              if (self.exBigTypeList[self.currentIndex].exCurrentFormCode == "8") {
  ...
  computed: {},
  watch: {
    // workCell: function(newVal,oldVal){
    //   this.workCell2 = newVal; 
    // },
    // workCenter: function(newVal,oldVal){
    //   this.workCenter2 = newVal; //newVal即是chartData
    // },
  },
  mounted() {
    this.getUsmBillStateByWorkCell();
