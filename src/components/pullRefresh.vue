<template>
    <div class="pull-refresh" ref="pullF" :style="{height: pullHeight}" >
		<div class="pull-info" :style="{height: tipHeight}" v-if="!refreshing" v-show="isShow">
			<img src="../assets/refresh_arrow.png" :style="{transform:`rotate(${arrowDeg}deg)`,transition:transition}"/>
			<span>{{pullTip}}</span>
		</div>
