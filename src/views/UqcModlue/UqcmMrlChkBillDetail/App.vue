    ...
    <mu-flex align-items="center" style="width:100%;padding:.5% 1%;">
      <fieldset style="width:100%; font-size:1.2rem;">
        ...
        <mu-flex align-items="center" fill>
          ...
          <mu-flex style="width:25%;">
            <span class="lableSpan">采购订单：</span>
            <span>{{checkBill.purchaseOrderCode}}</span>
          </mu-flex>
          <mu-flex style="width:25%;">
            <span class="lableSpan">物料编码：</span>
            <span>{{checkBill.mrlCode}}</span>
          </mu-flex>
          <mu-flex style="width:25%;">
            <span class="lableSpan">物料名称：</span>
            <span>{{checkBill.mrlName}}</span>
          </mu-flex>
        </mu-flex>
        <mu-flex align-items="center" fill>
          <mu-flex style="width:25%;">
            <span class="lableSpan">ASN单号：</span>
            <span>{{checkBill.asnCode}}</span>
          </mu-flex>
          <mu-flex style="width:25%;">
            <span class="lableSpan">收货单号：</span>
            <span>{{checkBill.receiptCode}}</span>
          </mu-flex>
          <mu-flex style="width:25%;">
            <span class="lableSpan">到货时间：</span>
            <span>{{checkBill.receiptTime}}</span>
          </mu-flex>
        </mu-flex>
        <mu-flex align-items="center" fill>
          <mu-flex style="width:25%;">
                  ...
                  <mu-flex><span class="lableSpan">检测值：</span>
                    ...
                    <div v-for="(item2,index2) in citemResult" :key="index2">
                      ...
                      <mu-radio @change="radioChange(item2.id,item3.id)" v-if="item.controlType == 2 && item2.chkBillCitemId == item.id && item3.citemResultId == item2.id" 
                        :value=2 v-model="item3.selectFlag" v-bind:disabled="isDisabled" class="finished" :label="item3.citemDName"></mu-radio>
