  ...
  <div class="app">
      ...
      <mu-list-item :ripple="false" class="input-item" style="text-align:center;">
        ...
        <mu-list-item-content>
          <section class="bottom-auto">
            <mu-row>
              <mu-col span="11">
                <mu-text-field type="text" v-model="scanCode" placeholder="请输入到货单号或ASN单号" style="width:100%;" v-on:keyup.enter="scanWorkOrder"></mu-text-field>
              </mu-col>
    ...
    <div id="checkBillContainer" v-for="(item,index) in checkBills" :key="index">
      ...
      <mu-row class="list-content" style="width: 100%;">
        ...
        <mu-col class="machine_name" span="3">采购订单：<span>{{item.purchaseOrderCode}}</span></mu-col>
        <mu-col class="machine_properties fontcolor" span="3">ASN单号：<span>{{item.asnCode}}</span></mu-col>
        <mu-col class="machine_properties fontcolor" span="3">收货单号：<span>{{item.receiptCode}}</span></mu-col>
        <mu-col class="machine_properties fontcolor" span="3">物料编码：<span>{{item.mrlCode}}</span></mu-col>
        <mu-col class="machine_properties fontcolor" span="3">物料名称：<span>{{item.mrlName}}</span></mu-col>
        <mu-col class="machine_properties fontcolor" span="3">&nbsp;&nbsp;&nbsp;批次号：<span>{{item.opCode}}</span></mu-col>
        <mu-col class="machine_properties fontcolor" span="3">到货时间：<span>{{item.receiptTime}}</span></mu-col>
      ...
      <div class="mu-dialog-body">
        <div class="input-form">
          <mu-row style="width: 100%;">
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">单号</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="scanAddCode" v-on:keyup.enter="searchByCode" placeholder="请输入单号" type="text" underline-color="#2cb69a"></mu-text-field></mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">检测模板</span></mu-col>
            <mu-col span="4">
              <mu-select filterable="" v-model="chkTemplateId" @change="chkTemplateChange">
                <mu-option v-for="(item,index) in chkTemplates" :key="index" :label="item.showName" :value="item.id"></mu-option>
              </mu-select>
            </mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">采购订单号</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="order.purchaseOrderCode" type="text" underline-color="#2cb69a" disabled></mu-text-field></mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">ASN单号</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="order.asnCode" type="text" underline-color="#2cb69a" disabled></mu-text-field></mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">收货单号</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="order.receiptCode" type="text" underline-color="#2cb69a" disabled></mu-text-field></mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">发货时间</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="order.receiptTime" type="text" underline-color="#2cb69a" disabled></mu-text-field></mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">物料编码</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="order.mrlCode" type="text" underline-color="#2cb69a" disabled></mu-text-field></mu-col>
            <mu-col span="2" style="text-align:right;padding-right:3%;"><span class="span-lable">物料名称</span></mu-col>
            <mu-col span="4"><mu-text-field class="text-input" v-model="order.mrlName" type="text" underline-color="#2cb69a" disabled></mu-text-field></mu-col>
...
export default {
  ...
  methods: {
    ...
    opAddCheckBillDialog(flag){
      if(flag == 1){
        this.order.mrlCode = "";
        this.getUqcmDefOpChkTemplateByParamMap();//获取检测模板
    ...
    searchByCode(){
      let data = qs.stringify({
        scanAddCode: this.scanAddCode.trim()
      });
      const loading = this.$loading({});
      http.post({url: "productPadCommController!getOrderInfo.m",
          data: data
        }).then(res => {
    ...
    getUqcmDefOpChkTemplateByParamMap(){
      let data = qs.stringify({
        ...
        mrlCode: this.order.mrlCode
      });
      const loading = this.$loading({});
      http.post({url: "productPadCommController!getUqcmChkTemplateByParamMap.m",
          data: data
        }).then(res => {
          if(loading) loading.close();
          this.woCode = "";
          if (res.status == 200 && res.data.erroCode == 0) {
            ...
            let resData = res.data.data || [];
            this.chkTemplates = resData;
            if(resData.length > 0){
              this.chkTemplateId = (resData[0]).id;
              if(!this.order.mrlCode || !this.order.mrlCode == ""){
                this.order.mrlCode = (resData[0]).mrlCode;
                this.order.mrlName = (resData[0]).mrlName;
              }
            }
          }else{
            Toast.error({
    ...
    chkTemplateChange(){
      if(!this.order.mrlCode || !this.order.mrlCode == ""){
        this.chkTemplates.forEach(item => {
          if(item.id == this.chkTemplateId) {
            this.order.mrlCode = item.mrlCode;
            this.order.mrlName = item.mrlName;
          }
        });
      }
    },
    ...
    addCheckBill(){
      ...
      const loading = this.$loading({});
      http.post({url: "productPadCommController!addMrlCheckBill.m",
          data: data
        }).then(res => {
          if(loading) loading.close();
          if (res.status == 200 && res.data.erroCode == 0) {
    ...
    goPage(checkBill) {
      ...
      this.$root.api.api.openWin({
        name: "UqcmMrlChkBillDetail",
        url: "UqcmMrlChkBillDetail.html",
        pageParam: {
          checkType: this.checkType,
    ...
    touchMove() {
      ...
      window.onmousemove = () => {
        if(this.mouseIsdown) this.isChangeTab = true;
      }
      //鼠标松开事件
      window.onmouseup = () => {
        let targ;
