<template>
  <div id="product">
    ...
    <div class="pro-top-menu" v-if="showFlag">
      ...
      <!--@update去掉 <div class="top-box" @click="toggleStatus('openTrans')">料车转运</div>
    ...
    <mu-paper class="demo-paper" :z-depth="1">
      <mu-container style="height:100%;width:100%;max-width:100%;">
        ...
        <div class="pro-demo pro-demo-product" v-if="active === 1">
          ...
          <div class="center" style="width:100%;height:39vh;border:none;margin: 20px auto;">
            ...
            <div class="center-left" style="width:52%;margin-left:1%;height:100%;border: 1px solid #666666;overflow:visible;border-radius: 15px;">
                <!-- 资源录入标题头 -->
                <span class="center-title" style='line-height:1.2rem;'>工艺资源信息填报</span>
          ...
          <div class="pro-shunxu">
            ...
            <!--@update去除 <div class="pro-shunxu-active" :disabled="ZZP_SJ"  @click="addCheckBill('ZZP_SJ')" >首检</div>
        ...
        <div class="pro-demo pro-demo-from" v-if="active === 3">
          <!-- 引入异常控制自定义组件 -->
          <UsmAndon :workCell="workCell" :workCenter="workCenter" :trackOrderId="form.id" :equip="equip" :tool="tool"></UsmAndon>
        </div>
      </mu-container>
    </mu-paper>
    ...
    <mu-container v-if="openFinshReport">
      <mu-dialog title="消息提醒" width="32%"  max-width="80%" :open.sync="openFinshReport" dialog-class="dialog-div"
        ...
        <mu-button slot="actions" flat color="primary" @click="doSubmit('finish')">确认</mu-button>
    ...
    <mu-container v-if="openAlert">
      <mu-dialog  title="报工采集"
        ...
        <div style="text-align:center;width:100%;">
          ...
          <mu-text-field class="text-input" v-model="form.discardQty" type="number" underline-color="#2cb69a"></mu-text-field><br/>
        </div>
...
<!-- 报废明细项 -->
          <div class="flex-item" style="width:100%;">
              <mu-divider></mu-divider> 
              <div class="flex-ul" style="width:100%;background-color:rgb(209, 209, 209);">
  ...
  methods: {
    ...
    toggleStatus(operState) {
      ...
      if (operState == "disableButton") {
        ...
        if ( (operState == "start" && this.kai == "报工") 
            || operState == "openTrans" || operState == "openMaterRack" 
            || operState == "openReportInfo" || operState == "openOpBindMrl") {
...
//获取不良明细
    ...
    doSubmit(type) {
      ...
      if (type == "report") {
        if(!this.checkReportQty()){ //报工时校验填报 良品/报废 数量
          return;
        }
        form.append("effectiveQty", this.form.goodQty); //良品数量
        form.append("uneffectiveQty", this.form.badQty); //不良品数量
        form.append("discardQty", this.formSuspiQtys); //报废数量  this.form.discardQty
        //报废不良项主键 及 明细数量
      ...
      this.checkIsAllowClick(type); //校验是否为正常操作
      ...
      // 发送请求:根据操作报工
      http.post({ url: "productPadCommController!doSubmitTheLast.m", data: form })
        .then(res => {
          if (res.status == 200 && res.data.erroCode == 0) {
            //按钮的启用和禁用
            this.queryCurWork();
  ...
  .container {
    ...
    .pro-demo {
      ...
      .pro-shunxu {
        ...
        div {
          float: left;
          width: 33%;//去除首末件按钮，@update 之前为 20%
          min-height: 40px;
          line-height: 40px;
          text-align: center;
